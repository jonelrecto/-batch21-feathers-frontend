  ...
  <div>
    ...
    <div class="q-pa-md">
      <div class="row">
        ...
        <q-btn @click="find" icon="search" round color="secondary"></q-btn>
      </div>
...
export default {
  components: {
    // inserting components on this page
    // rHello
  },
  beforeDestroy () {
    // kill all event listeners before closing this page
    // eq. closing all connections
  },
  mounted () {
    this.$dbCon.services.tasks.onDataChange(data => {
      this.todos = data
      console.log('Hello')
    })
    // console.log(this.$dbCon.passport.verifyJWT(), 'dbcon')
    this.$dbCon.authenticate()
    this.$dbCon.on('authenticated', (res) => {
      // console.log(accessToken, 'token')
      this.$dbCon.passport.verifyJWT(res.accessToken)
        .then(res => {
          console.log(res, 'res')
        })
        .catch(err => {
  ...
  methods: {
    ...
    showOngoing () {
      this.$dbCon.services.tasks.reQuery({
        query: {
          ...
          $sort: {
            // -1 for latest todos added
            // 0
            // 1 for alphabetical
            createdAt: -1
          }
        },
        // reactivity to change status
        channels: [
          {
            value: false,
            prop: 'done'
    ...
    addTask () {
      // this.todos.push({ ...this.task }) spread the data (...)
    ...
    gehgeh () {
      // this.todos.splice(this.selected, 1) get an id to delete
      console.log(this.selected, 'selected to delete')
